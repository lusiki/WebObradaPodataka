...
idf %>%
  arrange(desc(tf_idf))  %>%
  filter(word != c("5400","family.hr","moj","50","4300","ti","1","ep4341","ea8170","mp")) %>%
...
```{r nGRAMI, message=F, warning=F, fig.height=8, fig.width=6}
...
brandovi <- c("de’longhi", "krups", "lattego", "nespresso")

corsWords %>%
  filter(item1 %in% brandovi) %>%
  group_by(item1) %>%
  top_n(20) %>%
  ungroup() %>%
